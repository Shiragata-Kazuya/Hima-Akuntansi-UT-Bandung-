...
const StrukturPage = (() => {
    ...
    const _renderPersonCard = (person, role, isLeader = false) => {
        ...
        // Prioritas: posX/posY (format baru dari admin panel)
        // Fallback: posisi CSS class (format lama), lalu default object-top
        let objPosition;
        if (person.posX != null && person.posY != null) {
            // Format baru: angka 0–100
            objPosition = `${person.posX}% ${person.posY}%`;
        } else if (person.posisi) {
            // Format lama: CSS class seperti 'object-top', 'object-center'
            // Konversi ke nilai % agar kompatibel dengan inline style
            const legacyMap = {
                'object-top'    : '50% 0%',
            ...
            objPosition = legacyMap[person.posisi] || '50% 20%';
        } else {
            objPosition = '50% 20%'; // default: sedikit ke atas agar wajah terlihat
        }

        const scale        = person.scale ?? 1;
        const transformOrigin = objPosition; // agar zoom berpusat di titik yang sama
            ...
            <div class="flex flex-col items-center group select-none">
                <div class="${size} rounded-full overflow-hidden border-4 ${border} shadow-lg mb-3 bg-white hover:shadow-xl transition-shadow cursor-pointer relative"
                    ...
                    <img src="${_e(person.img)}" alt="${_e(person.name)}"
                         ...
                         style="object-position:${objPosition}; transform:scale(${scale}); transform-origin:${transformOrigin}; transition:transform 0.3s ease;"
                         loading="lazy"
